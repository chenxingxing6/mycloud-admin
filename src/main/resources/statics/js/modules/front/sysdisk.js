$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/sysdisk/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '企业名字', name: 'companyName', index: 'company_name', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.sysDisk.id == null ? "sys/sysdisk/save" : "sys/sysdisk/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/sysdisk/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/sysdisk/info/"+id, function(r){
                vm.sysDisk = r.sysDisk;
            });
		},
